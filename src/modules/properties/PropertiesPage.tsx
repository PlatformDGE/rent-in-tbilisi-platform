...
export default function PropertiesPage() {
  ...
  const districts = [...new Set(properties.map((item) => item.district).filter(Boolean))].sort();
  const agents = [...new Set(properties.map((item) => item.agent).filter(Boolean))].sort();
  const types = [...new Set(properties.map((item) => item.propertyType))].sort();
  const filtered = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase('ru');
    return properties.filter((item) => (!needle || [item.internalId, item.address, item.cadastralNumber, item.ownerName, item.ownerPhone].some((value) => value.toLocaleLowerCase('ru').includes(needle))) && (!status || item.status === status) && (!district || item.district === district) && (!source || item.source === source) && (!agent || item.agent === agent) && (!type || item.propertyType === type) && (!currency || item.currency === currency)).sort((a, b) => {
...
function PropertyTable({items}:{items:Property[]}) { return <div className="property-table-wrap"><table className="property-table"><thead><tr><th>Объект</th><th>Статус</th><th>Цена</th><th>Район</th><th>Агент</th><th>Источник</th><th>Обновлён</th></tr></thead><tbody>{items.map(item=><tr key={item.id}><td><Link to={`/properties/${item.id}`}><strong>{item.internalId}</strong><small>{item.address}</small></Link></td><td><span className={statusClass(item.status)}>{item.status}</span></td><td>{formatPrice(item.price,item.currency)}</td><td>{item.district}</td><td>{item.agent||'Не назначен'}</td><td>{item.source}</td><td>{formatDate(item.updatedAt)}</td></tr>)}</tbody></table></div>; }
function PropertyCard({item}:{item:Property}) { return <Link className="property-card" to={`/properties/${item.id}`}><div className="property-card__top"><strong>{item.internalId}</strong><span className={statusClass(item.status)}>{item.status}</span></div><h3>{item.address}</h3><p>{item.district} · {item.propertyType}</p><div className="property-card__price">{formatPrice(item.price,item.currency)}</div><dl><div><dt>Агент</dt><dd>{item.agent||'Не назначен'}</dd></div><div><dt>Источник</dt><dd>{item.source}</dd></div><div><dt>Обновлён</dt><dd>{formatDate(item.updatedAt)}</dd></div></dl></Link>; }
